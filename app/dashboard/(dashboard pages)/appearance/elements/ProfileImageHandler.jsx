...
export default function ProfileImageManager() {
  ...
  const handleUploadPhoto = async () => {
    ...
    const photo = `${generateUniqueId()}.${uploadedPhoto.name.substring(
      uploadedPhoto.name.lastIndexOf(".") + 1
    )}`;
  ...
  return (
    <div className="flex w-full p-6 items-center gap-4">
      <div
        className="h-[6rem] w-[6rem] cursor-pointer rounded-full grid place-items-center border overflow-hidden"
      ...
      <div className="flex-1 grid gap-2 relative">
        ...
        />
        <div
          className={`flex items-center gap-3 justify-center p-3 rounded-3xl cursor-pointer active:scale-95 active:opacity-60 active:translate-y-1 hover:scale-[1.005] bg-btnPrimary text-white w-full`}
          onClick={() => inputRef.current.click()}
        >
          Pick an image
        </div>
        <div
          className={`flex items-center gap-3 justify-center p-3 rounded-3xl mix-blend-multiply cursor-pointer active:scale-95 active:opacity-60 active:translate-y-1 hover:scale-[1.005] border w-full`}
