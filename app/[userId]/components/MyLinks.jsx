...
export default function MyLinks({ userId, hasSensitiveContent }) {
  ...
  useEffect(() => {
    setDisplayLinks(myLinksArray.filter((link) => link.isActive !== false));
  }, [myLinksArray]);

  return (
    <div
      className={`flex flex-col gap-4 my-4 w-full px-5 py-1 items-center max-h-fit ${
        supportGroupStatus ? "pb-12" : ""
      }`}
    >
      {socialPosition === 0 && socialArray.length > 0 && (
        <Socials themeFontColor={themeFontColor} socialArray={socialArray} />
      )}
        ...
        if (link.type === 0) {
          return (
            <span
              ...
              style={{
                color: `${
                  themeFontColor === "#000" ? themeTextColour : themeFontColor
                }`,
              }}
